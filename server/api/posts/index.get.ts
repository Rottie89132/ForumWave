const transformData = async (data: any[], userId: any): Promise<any[]> => {
    ...
    return data.map( (item: any) => {
        ...
        const title = headingItem && headingItem.content && headingItem.content.length > 0 ? headingItem.content[0].text : 'No title';
        ...
        const media = itemdata.find((item: any) => item.type === 'image')?.attrs.src || '/Post.png';
...
export default defineEventHandler(async (event) => {
    ...
    if (popular) posts = await Posts.find({ "meta.Likes": { $gt: 0 } }).sort({ "meta.Likes": -1 }).skip(skip).limit(itemsPerPage);
    else posts = await Posts.find({ "meta.Likes": 0 }).sort({ CreatedAt: -1 }).skip(skip).limit(itemsPerPage);
